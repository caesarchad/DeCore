...
impl<T: Clone> StatusCache<T> {
    ...
    pub fn get_signature_status(
        &self,
        sig: &Signature,
        transaction_transaction_seal: &Hash,
        ancestors: &HashMap<ForkId, usize>,
    ) -> Option<(ForkId, T)> {
        let (_, index, sigmap) = self.cache.get(transaction_transaction_seal)?;
    ...
    /// TODO: wallets should send the Transactions recent transaction_seal as well
    pub fn get_signature_status_slow(
        ...
        for transaction_seal in self.cache.keys() {
            trace!("get_signature_status_slow: trying {}", transaction_seal);
            if let Some((forkid, res)) = self.get_signature_status(sig, transaction_seal, ancestors) {
    ...
    pub fn insert(&mut self, transaction_transaction_seal: &Hash, sig: &Signature, fork: ForkId, res: T) {
        let index: usize =
            thread_rng().gen_range(0, std::mem::size_of::<Hash>() - CACHED_SIGNATURE_SIZE);
        let sig_map =
            self.cache
                .entry(*transaction_transaction_seal)
...
mod tests {
    ...
    fn test_empty_has_no_sigs() {
        ...
        assert_eq!(
            status_cache.get_signature_status(&sig, &transaction_seal, &HashMap::new()),
            None
        );
        assert_eq!(
    ...
    fn test_find_sig_with_ancestor_fork() {
        ...
        status_cache.insert(&transaction_seal, &sig, 0, ());
        assert_eq!(
            status_cache.get_signature_status(&sig, &transaction_seal, &ancestors),
            Some((0, ()))
        );
        assert_eq!(
    ...
    #[test]
    fn test_insert_picks_latest_transaction_seal_fork() {
        let sig = Signature::default();
        let mut status_cache = TreasuryStatusCache::default();
        let transaction_seal = hash(Hash::default().as_ref());
        let ancestors = vec![(0, 0)].into_iter().collect();
        status_cache.insert(&transaction_seal, &sig, 0, ());
        status_cache.insert(&transaction_seal, &sig, 1, ());
        for i in 0..(MAX_CACHE_ENTRIES + 1) {
            status_cache.add_root(i as u64);
        }
        assert!(status_cache
            .get_signature_status(&sig, &transaction_seal, &ancestors)
            .is_some());
    }
    ...
    fn test_signatures_slice() {
        ...
        status_cache.clear_signatures();
        status_cache.insert(&transaction_seal, &sig, 0, ());
        let (_, index, sig_map) = status_cache.cache.get(&transaction_seal).unwrap();
        let mut sig_slice = [0u8; CACHED_SIGNATURE_SIZE];
        sig_slice.clone_from_slice(&sig.as_ref()[*index..*index + CACHED_SIGNATURE_SIZE]);
