...
/// let keypairs: Vec<Keypair> = (0..64).map(|_| Keypair::create(&mut csprng)).collect();
...
impl Keypair {
    ...
    ///   (As obtained from `Keypair::to_octets()`.)
    ///
    /// # Warning
    ///
    ...
    ///
    /// let sig: Signature = keypair.sign_already_hashed(prehashed, Some(context));
    /// # }
    /// #
    /// # #[cfg(not(feature = "std"))]
    /// # fn main() { }
    ...
    ///
    /// let verified = keypair.public.validate_already_hashed(prehashed_again, Some(context), &sig);
    ///
