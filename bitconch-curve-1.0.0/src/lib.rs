...
//!
//! First, we need to create a `Keypair`, which includes both public and
...
//! ```
//!
//! As well as to validate that this is, indeed, a valid signature on
//! that `message`:
//!
//! ```
//! # extern crate rand;
...
//! # let message: &[u8] = b"This is a test of the tsunami alert system.";
//! # let signature: Signature = keypair.sign(message);
//! assert!(keypair.validate(message, &signature).is_ok());
//! # }
//! ```
//!
...
//! assert!(public_key.validate(message, &signature).is_ok());
//! # }
//! ```
//!
//! ## Serialisation
...
//! into byte-arrays by calling `.to_octets()`.  It's perfectly acceptible and
...
//! # let public_key: PublicKey = keypair.public;
//!
//! let public_key_bytes: [u8; PUBLIC_KEY_LENGTH] = public_key.to_octets();
//! let secret_key_bytes: [u8; SECRET_KEY_LENGTH] = keypair.secret.to_octets();
//! let keypair_bytes:    [u8; KEYPAIR_LENGTH]    = keypair.to_octets();
//! let signature_bytes:  [u8; SIGNATURE_LENGTH]  = signature.to_octets();
//! # }
//! ```
//!
//! And similarly, decoded from bytes with `::from_octets()`:
//!
//! ```
//! # extern crate rand;
//! # extern crate ed25519_dalek;
...
//! # let mut csprng = thread_rng();
//! # let keypair_orig: Keypair = Keypair::create(&mut csprng);
//! # let message: &[u8] = b"This is a test of the tsunami alert system.";
//! # let signature_orig: Signature = keypair_orig.sign(message);
//! # let public_key_bytes: [u8; PUBLIC_KEY_LENGTH] = keypair_orig.public.to_octets();
//! # let secret_key_bytes: [u8; SECRET_KEY_LENGTH] = keypair_orig.secret.to_octets();
//! # let keypair_bytes:    [u8; KEYPAIR_LENGTH]    = keypair_orig.to_octets();
//! # let signature_bytes:  [u8; SIGNATURE_LENGTH]  = signature_orig.to_octets();
//! #
//! let public_key: PublicKey = PublicKey::from_octets(&public_key_bytes)?;
//! let secret_key: SecretKey = SecretKey::from_octets(&secret_key_bytes)?;
//! let keypair:    Keypair   = Keypair::from_octets(&keypair_bytes)?;
//! let signature:  Signature = Signature::from_octets(&signature_bytes)?;
//! #
...
//! # let signature: Signature = keypair.sign(message);
//! # let public_key: PublicKey = keypair.public;
//! # let verified: bool = public_key.validate(message, &signature).is_ok();
//!
...
//! recipient may deserialise them and validate:
//!
//! ```
//! # extern crate rand;
//! # extern crate ed25519_dalek;
...
//! #
//! let verified: bool = decoded_public_key.validate(&message, &decoded_signature).is_ok();
//!
//! assert!(verified);
//! # }
//! # #[cfg(not(feature = "serde"))]
