...
impl SecretKey {
    ...
    /// let secret_key: SecretKey = SecretKey::create(&mut csprng);
    /// # }
    /// #
    /// # #[cfg(not(feature = "std"))]
    /// # fn main() { }
    /// ```
    ///
    /// Afterwards, you can create the corresponding public:
    ///
    /// ```
    /// # extern crate rand;
    /// # extern crate ed25519_dalek;
...
impl ExpandedSecretKey {
    ...
    /// let secret_key: SecretKey = SecretKey::create(&mut csprng);
    /// let expanded_secret_key: ExpandedSecretKey = ExpandedSecretKey::from(&secret_key);
    /// let expanded_secret_key_bytes: [u8; 64] = expanded_secret_key.to_octets();
    ///
    ...
    /// let secret_key: SecretKey = SecretKey::create(&mut csprng);
    /// let expanded_secret_key: ExpandedSecretKey = ExpandedSecretKey::from(&secret_key);
    /// let bytes: [u8; 64] = expanded_secret_key.to_octets();
    /// let expanded_secret_key_again = ExpandedSecretKey::from_octets(&bytes)?;
    /// #
