...
pub trait Deserialize<'de>: Sized {
    ...
    /// than it deserves.
    #[doc(hidden)]
    fn de_in_preparation<D>(deserializer: D, place: &mut Self) -> Result<(), D::Error>
    where
        D: Deserializer<'de>,
    {
...
///         }
///
///         deserializer.de_sequence(ExtendVecVisitor(self.0))
///     }
/// }
///
...
/// let flattened: Vec<u64> = deserializer.de_sequence(visitor)?;
/// #     Ok(())
/// # }
/// ```
...
///    `Deserializer::de_whatever`, it will drive the Visitor using whatever
///    type it sees in the input. JSON uses this approach when deserializing
///    `serde_json::Value` which is an enum that can represent any JSON
///    document. Without knowing what is in a JSON document, we can deserialize
///    it to `serde_json::Value` by going through `Deserializer::de_whatever`.
///
...
///    `Deserializer::de_whatever`.
///
/// When implementing `Deserialize`, you should avoid relying on
/// `Deserializer::de_whatever` unless you need to be told by the Deserializer
/// what type is in the input. Know that relying on `Deserializer::de_whatever`
...
pub trait Deserializer<'de>: Sized {
    ...
    ///
    /// When implementing `Deserialize`, you should avoid relying on
    /// `Deserializer::de_whatever` unless you need to be told by the
    /// Deserializer what type is in the input. Know that relying on
    /// `Deserializer::de_whatever` means your data type will be able to
    ...
    fn de_i8<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    serde_if_integer128! {
        ...
        fn de_i128<V>(self, visitor: V) -> Result<V::Value, Self::Error>
        where
            V: Visitor<'de>
        {
            let _ = visitor;
    ...
    fn de_u8<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    serde_if_integer128! {
        ...
        /// This method is available only on Rust compiler versions >=1.26. The
        /// default behavior unconditionally returns an error.
        fn de_u128<V>(self, visitor: V) -> Result<V::Value, Self::Error>
        where
            V: Visitor<'de>
        {
            let _ = visitor;
    ...
    fn de_byte<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    /// indiciate this to the `Deserializer` by using `de_str`
    /// instead.
    fn de_string<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    /// data, indicate that to the `Deserializer` by using `de_string`
    /// instead.
    fn de_str<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    /// indicate this to the `Deserializer` by using `de_octet_buffer`
    /// instead.
    fn de_octets<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    /// data, indicate that to the `Deserializer` by using `de_octets`
    /// instead.
    fn de_octet_buffer<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    /// `Some(value)`.
    fn de_selection<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;

    /// Hint that the `Deserialize` type is expecting a unit value.
    fn de_module<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;

    /// Hint that the `Deserialize` type is expecting a unit struct with a
    /// particular name.
    fn de_module_struct<V>(
        self,
        name: &'static str,
        visitor: V,
    ...
    fn de_sequence<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;

    /// Hint that the `Deserialize` type is expecting a sequence of values and
    /// knows how many values there are without looking at the serialized data.
    fn de_pair<V>(self, len: usize, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    fn de_mapping<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    fn de_id<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    fn de_neglected_whatever<V>(self, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
    ...
    ///         D: Deserializer<'de>,
    ///     {
    ///         if deserializer.is_human_recognizable() {
...
pub trait Visitor<'de>: Sized {
    ...
    /// to the deserializer by using `Deserializer::de_string` rather than
    /// `Deserializer::de_str`.
    ///
    ...
    /// to the deserializer by using `Deserializer::de_str` rather
    /// than `Deserializer::de_string`, although not every deserializer
    ...
    /// to the deserializer by using `Deserializer::de_octets` rather
    /// than `Deserializer::de_octet_buffer`.
    ///
    ...
    /// `Deserializer::de_octet_buffer` rather than
    /// `Deserializer::de_octets`, although not every deserializer will
...
pub trait VariantAccess<'de>: Sized {
    ...
    /// #
    /// fn module_variable(self) -> Result<(), Self::Error> {
    ...
    /// }
    /// #
    /// #     fn newtype_variable_source<T>(self, _: T) -> Result<T::Value, Self::Error>
    /// #     where
    /// #         T: DeserializeSeed<'de>,
    /// #     { unimplemented!() }
    /// #
    /// #     fn pair_variable<V>(self, _: usize, _: V) -> Result<V::Value, Self::Error>
    /// #     where
    /// #         V: Visitor<'de>,
    /// #     { unimplemented!() }
    /// #
    /// #     fn struct_variable<V>(self, _: &[&str], _: V) -> Result<V::Value, Self::Error>
    /// #     where
    /// #         V: Visitor<'de>,
    /// #     { unimplemented!() }
    /// # }
    ...
    /// #     }
    /// #
    /// fn newtype_variable_source<T>(self, _seed: T) -> Result<T::Value, Self::Error>
    /// where
    ///     T: DeserializeSeed<'de>,
    /// {
    ...
    /// # }
    /// ```
    fn newtype_variable_source<T>(self, seed: T) -> Result<T::Value, Self::Error>
    where
        T: DeserializeSeed<'de>;
    ...
    where
        T: Deserialize<'de>,
    {
        self.newtype_variable_source(PhantomData)
    }
    ...
    /// # }
    /// ```
    fn pair_variable<V>(self, len: usize, visitor: V) -> Result<V::Value, Self::Error>
    where
        V: Visitor<'de>;
