...
fn audit_candidate_fscl_stmts(block_buffer_pool: &BlockBufferPool, slot: u64, tail_fscl_stmt: &Hash) -> Vec<FsclStmt> {
    let fscl_stmts = block_buffer_pool.fetch_candidate_fscl_stmts(slot, 0, None).unwrap();
    assert!(fscl_stmts.verify(tail_fscl_stmt));
    fscl_stmts
}
